...
export default function createToolbarAndroidComponent(IconNamePropType, getImageSource) {
  return class IconToolbarAndroid extends PureComponent {
    ...
    updateIconSources(props) {
      ...
      if (props.logoName) {
        getImageSource(props.logoName, size, color).then(logo => this.setState({ logo }));
      }
      ...
      Promise.all((props.actions || []).map(action => {
        if (action.iconName) {
          return getImageSource(action.iconName, action.iconSize || size, action.iconColor || color).then(icon => ({ ...action, icon }));
        }
