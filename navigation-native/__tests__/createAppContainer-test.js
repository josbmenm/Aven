...
import createAppContainer, { _TESTING_ONLY_reset_container_count } from '../createAppContainer';

import { NavigationActions, createNavigator, StackRouter, SwitchView } from '../../navigation-core';
...
describe('NavigationContainer', () => {
  ...
  describe('dispatch', () => {
    ...
    it('does not discard actions when called twice in one tick', () => {
      const navigationContainer = renderer.create(<NavigationContainer />).getInstance();
      const initialState = JSON.parse(JSON.stringify(navigationContainer.state.nav));
      ...
      expect(navigationContainer.state.nav).toMatchObject({
        index: 2,
        routes: [{ routeName: 'foo' }, { routeName: 'bar' }, { routeName: 'baz' }]
      });
    });
    ...
    it('does not discard actions when called more than 2 times in one tick', () => {
      ...
      expect(navigationContainer.state.nav).toMatchObject({
        index: 5,
        routes: [{ routeName: 'foo' }, { routeName: 'bar' }, { routeName: 'baz' }, { routeName: 'car' }, { routeName: 'dog' }, { routeName: 'elk' }]
      });
    });
  });
  ...
  describe('warnings', () => {
    ...
    describe('detached navigators', () => {
      ...
      it('warns when you render more than one container explicitly', () => {
        ...
        const ChildNavigator = createAppContainer(createStackNavigator({
          Child: BlankScreen
        }));
