...
module.exports = {
  ...
  applyPackage: async ({ location, appName, srcDir, distPkg, appPkg }) => {
    ...
    let outDistPkg = {
      ...
      scripts: {
        ...distPkg.scripts,
        'aven:deploy': `npx appcenter codepush release-react -a ${codePushApp} -d ${codePushChannel} --plist-file ios/thunder/Info.plist`,
      },
    };
    await fs.writeFile(distPkgPath, JSON.stringify(outDistPkg, null, 2));

    const xprojPath = `${location}/ios/thunder.xcodeproj/project.pbxproj`;
    ...
    const infoPath = `${location}/ios/thunder/Info.plist`;
...
AppRegistry.registerComponent('thunder', () => App);
`,
    );
    await spawn('yarn', [], {
      cwd: location,
