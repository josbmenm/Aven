...
Object.keys(ROUTERS).forEach(routerName => {
  ...
  describe(`General router features - ${routerName}`, () => {
    ...
    test(`set params works in ${routerName}`, () => {
      ...
      const state0 = router.getStateForAction(NavigationActions.setParams({
        params: { foo: 42 },
        key: initRoute.key
      }), initState);
...
test('Nested navigate behavior test', () => {
  ...
  const state2 = TestRouter.getStateForAction({ type: NavigationActions.NAVIGATE, routeName: 'First' }, state1);
  expect(state2.index).toEqual(1);
  expect(state2.routes[1].index).toEqual(0);
  expect(state2.routes[1].routes[0].index).toEqual(0);

  const state3 = TestRouter.getStateForAction({ type: NavigationActions.NAVIGATE, routeName: 'Second2' }, state2);
