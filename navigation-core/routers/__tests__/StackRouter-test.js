...
describe('StackRouter', () => {
  ...
  test('pop action works as expected', () => {
    ...
    const state = {
      index: 3,
      isTransitioning: false,
      routes: [{ key: 'A', routeName: 'foo' }, { key: 'B', routeName: 'bar', params: { bazId: '321' } }, { key: 'C', routeName: 'foo' }, { key: 'D', routeName: 'bar' }]
    };
    ...
    const poppedState2 = TestRouter.getStateForAction(StackActions.pop({ n: 2, immediate: true }), state);
    expect(poppedState2.routes.length).toBe(2);
    expect(poppedState2.index).toBe(1);
    expect(poppedState2.isTransitioning).toBe(false);

    const poppedState3 = TestRouter.getStateForAction(StackActions.pop({ n: 5 }), state);
  ...
  test('popToTop works as expected', () => {
    ...
    const state = {
      index: 2,
      isTransitioning: false,
      routes: [{ key: 'A', routeName: 'foo' }, { key: 'B', routeName: 'bar', params: { bazId: '321' } }, { key: 'C', routeName: 'foo' }]
    };
    const poppedState = TestRouter.getStateForAction(StackActions.popToTop(), state);
    expect(poppedState.routes.length).toBe(1);
    expect(poppedState.index).toBe(0);
    expect(poppedState.isTransitioning).toBe(true);
    const poppedState2 = TestRouter.getStateForAction(StackActions.popToTop(), poppedState);
    expect(poppedState).toEqual(poppedState2);
    const poppedImmediatelyState = TestRouter.getStateForAction(StackActions.popToTop({ immediate: true }), state);
  ...
  test('Navigate does not push duplicate routeName', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const barState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar' }), initState);
    expect(barState.index).toEqual(1);
    expect(barState.routes[1].routeName).toEqual('bar');
    const navigateOnBarState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar' }), barState);
  ...
  test('Navigate focuses given routeName if already active in stack', () => {
    ...
    const fooBarState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar' }), initialState);
    const fooBarBazState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'baz' }), fooBarState);
    expect(fooBarBazState.index).toEqual(2);
    expect(fooBarBazState.routes[2].routeName).toEqual('baz');

    const fooState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'foo' }), fooBarBazState);
  ...
  test('Navigate pushes duplicate routeName if unique key is provided', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const pushedState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar' }), initState);
    expect(pushedState.index).toEqual(1);
    expect(pushedState.routes[1].routeName).toEqual('bar');
    const pushedTwiceState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar', key: 'new-unique-key!' }), pushedState);
  ...
  test('Navigate to initial screen is possible', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const pushedState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'foo', key: 'foo' }), initState);
  ...
  test('Navigate with key and without it is idempotent', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const pushedState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar', key: 'a' }), initState);
    expect(pushedState.index).toEqual(1);
    expect(pushedState.routes[1].routeName).toEqual('bar');
    const pushedTwiceState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar', key: 'a' }), pushedState);
  ...
  test('Navigate on inactive stackrouter is idempotent', () => {
    ...
    const first = router.getStateForAction(NavigationActions.navigate({ routeName: 'First2' }), state);

    const second = router.getStateForAction(NavigationActions.navigate({ routeName: 'Second2' }), first);

    const firstAgain = router.getStateForAction(NavigationActions.navigate({
      routeName: 'First2',
      params: { debug: true }
    }), second);
  ...
  test('Navigate to current routeName returns null to indicate handled action', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const navigatedState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'foo' }), initState);
  ...
  test('Push behaves like navigate, except for key', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const pushedState = TestRouter.getStateForAction(StackActions.push({ routeName: 'bar' }), initState);
    expect(pushedState.index).toEqual(1);
    expect(pushedState.routes[1].routeName).toEqual('bar');
    expect(() => {
      TestRouter.getStateForAction({ type: StackActions.PUSH, routeName: 'bar', key: 'a' }, pushedState);
    }).toThrow();
  });
  ...
  test('Push adds new routes every time', () => {
    ...
    expect(pushedState.index).toEqual(1);
    expect(pushedState.routes[1].routeName).toEqual('bar');
    const secondPushedState = TestRouter.getStateForAction(StackActions.push({ routeName: 'bar' }), pushedState);
  ...
  test('Navigate backwards with key removes leading routes', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.init());
    const pushedState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar', key: 'a' }), initState);
    const pushedTwiceState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'bar', key: 'b`' }), pushedState);
    const pushedThriceState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'foo', key: 'c`' }), pushedTwiceState);
    expect(pushedThriceState.routes.length).toEqual(4);

    const navigatedBackToFirstRouteState = TestRouter.getStateForAction(NavigationActions.navigate({
      routeName: 'foo',
  ...
  test('Handle basic stack logic for plain components', () => {
    ...
    const state3 = router.getStateForAction({ type: NavigationActions.BACK, immediate: true }, state2);
  ...
  test('Replace action works', () => {
    ...
    const initState = TestRouter.getStateForAction(NavigationActions.navigate({ routeName: 'foo' }));
    const replacedState = TestRouter.getStateForAction(StackActions.replace({
      routeName: 'bar',
      params: { meaning: 42 },
    ...
    const replacedState2 = TestRouter.getStateForAction(StackActions.replace({
      routeName: 'bar',
      key: initState.routes[0].key,
      newKey: 'wow'
    }), initState);
  ...
  test('Replace action returns most recent route if no key is provided', () => {
    ...
    const replacedState = router.getStateForAction(StackActions.replace({
      routeName: 'Woo',
      params: { meaning: 42 }
    }), state5);
    ...
    expect(replacedCurrentScreen.routeName).not.toEqual(originalCurrentScreen.routeName);
  ...
  test('Back action parent is prioritized over inactive child routers', () => {
    ...
    const state = {
      key: 'top',
      index: 3,
      routes: [{ routeName: 'foo', key: 'f' }, {
        routeName: 'bar',
        key: 'b',
        index: 1,
        routes: [{ routeName: 'baz', key: 'bz' }, { routeName: 'qux', key: 'bx' }]
      }, { routeName: 'foo', key: 'f1' }, { routeName: 'boo', key: 'z' }]
    };
    const testState = TestRouter.getStateForAction({ type: NavigationActions.BACK }, state);
  ...
  test('Handles the SetParams action', () => {
    ...
    const state2 = key && router.getStateForAction({
  ...
  test('Handles the navigate action with params and nested StackRouter', () => {
    ...
    expect(state2 && state2.routes[1].routes).toEqual([expect.objectContaining({
      routeName: 'Baz',
      params: { foo: '42' }
    })]);
  });
