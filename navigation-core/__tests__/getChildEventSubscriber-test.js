...
test('child action events only flow when focused', () => {
  ...
  const childEventSubscriber = getChildEventSubscriber(parentSubscriber, 'key1').addListener;
  const testState = {
    key: 'foo',
    routeName: 'FooRoute',
...
test('grandchildren subscription', () => {
  ...
  const parentSubscriber = getChildEventSubscriber(grandParentSubscriber, 'parent').addListener;
  const childEventSubscriber = getChildEventSubscriber(parentSubscriber, 'key1').addListener;
...
const setupEventTest = (subscriptionKey, initialLastFocusEvent) => {
  ...
  const subscriptionRemove = () => {};
  parentSubscriber.mockReturnValueOnce({ remove: subscriptionRemove });
  const evtProvider = getChildEventSubscriber(parentSubscriber, subscriptionKey, initialLastFocusEvent);
  const handlers = {};
