...
class LinkWithNavigation extends Component {
  render() {
    ...
    const navAction = action || NavigationActions.navigate({
      routeName,
      key: routeKey,
      params
    });
    ...
    const href = Object.keys(pathAndParams.params).length ? `/${pathAndParams.path}?${queryString.stringify(pathAndParams.params)}` : `/${pathAndParams.path}`;
